...
[uv](https://docs.astral.sh/uv/concepts/tools/)가 설치되어 있다면 패키지를 영구 설치하지 않고 실행할 수도 있습니다:
...
MCP 클라이언트에서 `uvx`를 사용할 때는 `"command":"uvx"`와
`"args":["--from","apple-music-playlists","am-mcp"]`를 설정하세요.
