...
def classify_vocality(title: str, *, lyrics_found: bool | None = None,
    ...
    三种信号的可信度差得很远，所以不该压成一个布尔：

      1. 标题写着 `Instrumental` / `伴奏` / `Off Vocal` —— 几乎必然。值得注意：
         这也是**唯一不需要联网**就能拿到的器乐信号。
      2. 任何歌词源真的返回了歌词 —— 必然有人声。这是最强的正面证据。
      3. 查过的歌词源**全都没有** —— 这是**弱**证据：可能真是器乐，也可能只是没收录。
      4. 时长 < 2 分钟 —— 多半是过场/前奏。

    ⚠️ `has_apple_lyrics=False` **单独不足以判定器乐**。实测 Apple 的歌词覆盖只有 65%，
    而缺口集中在原声/爵士这类确实常为器乐的流派——但也可能是版权或未收录。
...
def vocality_report(counts: dict, total: int) -> str:
    ...
    if total <= 0:
        return "人声/器乐判定：（没有曲目）"
    order = (VOCAL, INSTRUMENTAL_MARKED, INSTRUMENTAL_UNMARKED, INTERLUDE, UNKNOWN)
    ...
    excluded = sum(int(counts.get(k, 0)) for k in
                   (INSTRUMENTAL_MARKED, INSTRUMENTAL_UNMARKED, INTERLUDE))
    if excluded:
